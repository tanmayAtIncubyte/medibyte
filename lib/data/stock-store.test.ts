...
// (all-or-nothing) and run-to-completion, so the clean path can never oversell
// or double-spend. The bug paths (unchecked / racy) deliberately break that.
...
describe("stock baseline — getAvailableStock", () => {
  it("starts at the seed stock", () => {
    expect(getAvailableStock(PRODUCT)).toBe(SEED);
  });

  it("returns 0 for an unknown product", () => {
    expect(getAvailableStock("prod-nope")).toBe(0);
  });
});

describe("stock baseline — reserveStock (atomic)", () => {
  it("decrements available stock on a successful reservation", () => {
    const result = reserveStock([{ productId: PRODUCT, quantity: 3 }]);
    expect(result.ok).toBe(true);
    expect(getAvailableStock(PRODUCT)).toBe(SEED - 3);
    expect(getReservedStock(PRODUCT)).toBe(3);
  });

  it("rejects an oversell and reserves nothing (all-or-nothing)", () => {
    const result = reserveStock([{ productId: PRODUCT, quantity: SEED + 1 }]);
  ...
  it("rejects the whole multi-line order if any one line oversells", () => {
    ...
    expect(getReservedStock("prod-ibuprofen-200")).toBe(0);
    expect(getAvailableStock(PRODUCT)).toBe(SEED);
  });

  it("two sequential reservations cannot exceed seed stock", () => {
    expect(reserveStock([{ productId: PRODUCT, quantity: 5 }]).ok).toBe(true);
    // Only 3 left; a second request for 5 must fail.
    expect(reserveStock([{ productId: PRODUCT, quantity: 5 }]).ok).toBe(false);
    expect(getReservedStock(PRODUCT)).toBe(5);
  });
});

describe("stock bug paths", () => {
  it("reserveStockUnchecked drives availability negative (oversell)", () => {
    const result = reserveStockUnchecked([{ productId: PRODUCT, quantity: SEED + 4 }]);
    expect(result.ok).toBe(true);
    expect(getReservedStock(PRODUCT)).toBe(SEED + 4); // more reserved than seeded
    expect(getAvailableStock(PRODUCT)).toBe(0); // clamped display, but oversold
  });
