...
| `FN_COUPON_NEGATIVE` | D | Discount not clamped to subtotal → **negative total** (e.g. $30 off a $20 cart → −$10.80). | `/cart`, `/checkout`, `/api/checkout` | Edge input: coupon worth more than cart (may need a seeded coupon) | No |
...
| `FN_CONCURRENT_DOUBLESPEND` † | X | Racy reservation snapshots availability, yields, then commits on the stale snapshot → two concurrent orders for the last units **both succeed** (double-spend). | `/api/checkout` (two parallel POSTs) | Concurrency / edge input: fire two checkouts in parallel for the last units, both succeed | No |
