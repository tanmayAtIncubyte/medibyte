# MediByte — Automation QA Assignment (6+ years experience)
...
1. Log in and go to the account page. Edit the existing saved address (find
   its "Edit ... address" control) and change the city, then save and
...
3. Add at least one product to the cart, then go to checkout and place an
   ...
   details.
4. Go to the order history page and confirm the order you just placed
   appears at the top of the list, with a status badge, item count, and
   total.
5. Open that order's detail page and cross-verify that the shipping address
   shown there — street, city, region, and postal code — exactly matches
   what you entered on the account page and at checkout.
6. With more than one order now in the history list, click into a specific
   order by its order ID and total (not by its position in the list) and
   confirm the detail page that opens shows that same order's ID, status,
   items, and total — not a different order's data.

This journey requires reading a value on one page, navigating away, and
comparing it against a value shown elsewhere — there's no single element
that tells you the check passed, so your automation needs to carry state
between pages/steps and assert equality across them.
