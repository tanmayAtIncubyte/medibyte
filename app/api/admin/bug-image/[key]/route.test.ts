...
//   - 404 for an unknown bug key (never reaches the disk),
//   - 400 for an invalid variant, and
//   - 200 + image/png for an admin when the screenshot exists.
//
// All 45 bugs now have committed clean+buggy PNGs in private/bug-shots/, so the
// 200 path is asserted against a real captured file. (The defensive ENOENT→404
// fallback for an as-yet-uncaptured key is exercised in practice pre-capture and
// is covered indirectly by the unknown-key 404 path.) getAdminOrNull is mocked.
...
describe("admin bug-image route — serving (MED-29)", () => {
  it("returns 200 + image/png for an admin when the screenshot exists", async () => {
    ...
    // FN_PRICE_DECIMALS-clean.png is a committed answer-key screenshot.
    const response = await GET(request("FN_PRICE_DECIMALS"), params("FN_PRICE_DECIMALS"));
